...
mod <- lm(ASSESSEDVAL ~   LANDSIZE + SALEPRICE + MEDIAN.INCOME + KITCHENS + FULLBATHS + HALFBATH + BEDROOMS + KITCHEN_FAC + COND_FAC + QUAL_FAC + TYPE_FAC + PRIM_WALL + salemonth + saleyear + age, data = fitdata)
...
mod <- lm(LogAssessedVal ~   LANDSIZE + LogSalesPrice + MEDIAN.INCOME + KITCHENS + FULLBATHS + HALFBATH + BEDROOMS + KITCHEN_FAC + COND_FAC + QUAL_FAC + TYPE_FAC + PRIM_WALL + salemonth + saleyear + age, data = fitdata)
...
# ok, so we see that Percent Black population always has a negative coefficient. This is also true in reference to the models with assessment value as the dependent variable

# This is saying as the percent black population increases, the assessment value decreases

# BUT, when including the Percent Not Hispanic White into the model there is no effect of any race variable. The interpretation of this is difficult. Basically, the Percent White is just the foil of the percent black. They are the majority populations, e.g. if one is 60 the other is close to 40

# So what is the interpretation of this? Should I include both in the model?


 LASSO MODELS
 
 The only useful information from this is that Number of Kitchens ought to be removed.
 
 With ATS Ratio there are different results.
...
I do not think we want assessment value in there

What we want are similar houses in



              # Observation Level Value Vars
              LogLandSize + LogSalesPrice + 
              
              # Track Level Vars
              LogMedianIncome  + LogMedianTractSalesPrice + LogMedianLandSize + 
              
              # Observation Level Characteristic Vars
              
    
```{r}

Blackobs <- resdata %>% filter(Majority_Black == 1) %>%
  select(LogAssessedVal, ATSDev_Indicator, ATSDev_Indicator_LOW,
         Percent_Black, Percent_Hispanic,
         LogLandSize, LogSalesPrice,
         LogMedianIncome, LogMedianTractSalesPrice, LogMedianLandSize,
