...
#setwd("/Users/vinniepalazeti/Desktop/GitGit/Milwaukee-Property-Assessment")
...
info <- resdata %>%
  group_by(GEO_TRACT) %>%
  summarize(MedianATSRatio = median(atsratio))
...
```{r}
resdata %>% select(SALEPRICE,MedianTractSalesPrice) %>% mutate(diff = ((SALEPRICE - MedianTractSalesPrice)/MedianTractSalesPrice)*100)
...
plot(resdata$salesdecile,resdata$atsratio)
...
```{r}
cors <- resdata %>% 
  select(MedianATSRatio, Percent_Black, Percent_White, Percent_Hispanic, Percent_POPBelowPovIncomeRatio, Percent_TotalPOPIncomeBelowPovRate, MEDIAN.INCOME, SALEPRICE, GEO_TRACT)
...
corrplot(cors,method='square', type = 'lower'); cors
