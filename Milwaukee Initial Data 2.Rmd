...
resdata <- resdata %>% mutate(BLDTYPE = ifelse(BLDTYPE == "C018 - Milwaukee Bungalow","18 - Milwaukee Bungalow",BLDTYPE))
...
Correlations
```{r}
cors <- resdata %>% select(MedianATSRatio,Percent_Black,Percent_White,Percent_Hispanic, Percent_POPBelowPovIncomeRatio, Percent_TotalPOPIncomeBelowPovRate, MEDIAN.INCOME, SALEPRICE, GEO_TRACT)

cors <- cors %>% distinct(GEO_TRACT, .keep_all = TRUE) %>% select(-c(GEO_TRACT))
