...
Fits Following Paper
```{r}
fitdata <- resdata %>% 
  select(atsratio,Percent_Black,Percent_Renter,Percent_POPBelowPovIncomeRatio,LogSalesPrice,SALEPRICE,LogMedianIncome,LogMedianTractSalesPrice,PriceDeviation,BLDTYPE,GEO_TRACT)

scaled.dat <- fitdata %>% 
  select(-c("BLDTYPE","GEO_TRACT")) %>%scale(center = T,scale=T)

fitdata <- as.data.frame(scaled.dat) %>% 
  mutate(GEO_TRACT = fitdata$GEO_TRACT,
         BLDTYPE = fitdata$BLDTYPE)

# for some reason Price Deviation blows up the model
mod1 <- lmer(atsratio ~ LogMedianIncome + Percent_Black + Percent_Renter + Percent_POPBelowPovIncomeRatio + LogMedianTractSalesPrice  + (1|GEO_TRACT), data = fitdata)
