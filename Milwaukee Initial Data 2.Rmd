...
resdata <- resdata %>% mutate(LANDSIZE = ifelse(LANDSIZEUNITS == "AC - Acres", LANDSIZE*43560,LANDSIZE))

table(resdata$LANDSIZEUNITS)

resdata$LogLandSize <- log(resdata$LANDSIZE)

TractAtsMedians <- resdata %>% group_by(GEO_TRACT) %>%
  summarise(atsratioMedian = median(atsratio))

resdata <- resdata %>% left_join(TractAtsMedians, by = c("GEO_TRACT"))
...
resdata <- resdata %>% left_join(info, by="GEO_TRACT") %>%
  mutate(PriceDeviation = (SALEPRICE - MedianTractSalesPrice),
         LandSizeDeviation = (LANDSIZE - MedianLandSize))


info <- resdata %>%
  group_by(GEO_TRACT) %>%
  summarise(MedianATSRatio = median(atsratio))
...
outofsample <- filter(Initial, atsratio <= le)
outofsample2 <- filter(Initial, atsratio >= ue)
outofsample <- rbind(outofsample,outofsample2)
...
subwhite %>% filter(PID %in% li2)
subblack %>% filter(PID %in% li)


ggplot(subblack, aes(SALEDATE)) +
...
BLDFreqs <- subblack %>% group_by(BLDTYPE) %>% summarise(n = n())
plotgroup <- subblack %>% left_join(BLDFreqs, by = c("BLDTYPE"))

ggplot(filter(plotgroup, n > 50), aes(BLDTYPE)) +
  geom_bar() + 
  xlab("Building Type") + 
  ylab("Frequency")

BLDFreqs <- subwhite %>% group_by(BLDTYPE) %>% summarise(n = n())
plotgroup <- subwhite %>% left_join(BLDFreqs, by = c("BLDTYPE"))
...
ggplot(subblack, aes(x = factor(QUAL_recode, level = level_order_qual))) +
  geom_bar() + 
  xlab("Quality") + 
  ylab("Frequency")
...
ggplot(subwhite, aes(x = factor(QUAL_recode, level = level_order_qual))) +
  geom_bar() + 
  xlab("Quality") + 
  ylab("Frequency")
...
whitepop <- tapply(subwhite$atsratio,subwhite$white_nonhispanic_decile, summary)
...
subblack %>% filter(LANDSIZE < 40000)
subwhite %>% filter(LANDSIZE > 40000)

ggplot(filter(subblack,LANDSIZE < 40000), aes(LANDSIZE)) +
  geom_histogram(bins = 200) +
  xlab("") + 
  ylab("Frequency") + 
  xlim(0, 40000) + 
...
ggplot(filter(subwhite,LANDSIZE < 40000), aes(LANDSIZE)) +
  geom_histogram(bins = 200) +
  xlab("") + 
  ylab("Frequency") + 
  xlim(0, 40000) + 
...
min(subwhite$LANDSIZE)


subblack %>% filter(LANDSIZE < 100)
subwhite %>% filter(LANDSIZE < 100)

table(subwhite$LANDSIZEUNITS)


subwhite %>% filter(LANDSIZEUNITS == "FF - Front Feet")


subwhite %>% group_by(PID) %>% summarize(n = n()) %>% filter(n >1)

subwhite %>% filter(PID == 162761)

# Propensity score matching Houses in 
```
...
#blquant=cut(resdata$Percent_Black, breaks = quantile(resdata$Percent_Black, p=seq(0,1,length.out = 8)), include.lowest = T)
